...
#define LOAD_USER_SHARED_DATA(reg) "movq " __ASM_NAME("user_shared_data") "(%rip)," reg "\n\t"
...
DECLSPEC_ALIGN(64) char iridium_fex_fallback_syscall_frame[0x10000];
DECLSPEC_ALIGN(64) char iridium_fex_fallback_user_stack[0x100000];
...
struct syscall_frame *iridium_fex_signal_start_syscall_frame( TEB *teb, void *rsp )
{
    struct ntdll_thread_data *thread_data = (struct ntdll_thread_data *)&teb->GdiTebBatch;
    struct syscall_frame *frame = thread_data->syscall_frame;
    INITIAL_TEB stack;

    if ((ULONG_PTR)frame >= 0x100000) return frame;
    if (!thread_data->kernel_stack &&
        virtual_alloc_thread_stack( &stack, limit_4g, 0, kernel_stack_size, kernel_stack_size, FALSE ) == STATUS_SUCCESS)
        thread_data->kernel_stack = stack.DeallocationStack;

    if (thread_data->kernel_stack)
        frame = (struct syscall_frame *)(((ULONG_PTR)thread_data->kernel_stack + kernel_stack_size - frame_size) & ~(ULONG_PTR)63);
    else
        frame = (struct syscall_frame *)(((ULONG_PTR)iridium_fex_fallback_syscall_frame + sizeof(iridium_fex_fallback_syscall_frame) - frame_size) & ~(ULONG_PTR)63);
...
void signal_init_process(void)
{
    ...
    kernel_stack = thread_data->kernel_stack ? (char *)thread_data->kernel_stack + kernel_stack_size :
        (char *)iridium_fex_fallback_syscall_frame + sizeof(iridium_fex_fallback_syscall_frame);
    thread_data->syscall_frame = (struct syscall_frame *)(((ULONG_PTR)kernel_stack - frame_size) & ~(ULONG_PTR)63);

    /* sneak in a syscall dispatcher thunk at a fixed address (7ffe1000) */
    ptr = (char *)user_shared_data + page_size;
    anon_mmap_fixed( ptr, page_size, PROT_READ | PROT_WRITE, 0 );
    *(BYTE *)ptr = 0xff;              /* jmpq *0(%rip) */
    *((BYTE *)ptr + 1) = 0x25;
    *(DWORD *)((BYTE *)ptr + 2) = 0;
    *(void **)((BYTE *)ptr + 6) = __wine_syscall_dispatcher;
...
void init_syscall_frame( LPTHREAD_START_ROUTINE entry, void *arg, BOOL suspend, TEB *teb )
{
    ...
    if ((ULONG_PTR)teb->Tib.StackBase < 0x100000 ||
        (ULONG_PTR)teb->Tib.StackBase == ~(ULONG_PTR)0 ||
        (teb->Tib.StackLimit && teb->Tib.StackBase <= teb->Tib.StackLimit))
    {
        INITIAL_TEB stack;
        if (virtual_alloc_thread_stack( &stack, 0, 0, 0x100000, 0x100000, TRUE ) == STATUS_SUCCESS)
        {
    ...
    if ((ULONG_PTR)teb->Tib.StackBase < 0x100000 ||
        ...
        (teb->Tib.StackLimit && teb->Tib.StackBase <= teb->Tib.StackLimit))
    {
        teb->Tib.StackLimit = iridium_fex_fallback_user_stack;
        teb->Tib.StackBase = iridium_fex_fallback_user_stack + sizeof(iridium_fex_fallback_user_stack);
        teb->DeallocationStack = teb->Tib.StackLimit;
    }
...
__ASM_GLOBAL_FUNC( signal_start_thread,
                   ...
                   "movq %rcx,%rdi\n\t"
                   "movq %rsp,%rsi\n\t"
                   "call " __ASM_NAME("iridium_fex_signal_start_syscall_frame") "\n\t"
                   "movq %rax,%r8\n\t"
                   "popq %rcx\n\t"
                   "popq %rdx\n\t"
                   "popq %rsi\n\t"
...
__ASM_GLOBAL_FUNC( __wine_syscall_dispatcher,
                   ...
                   "jmp 4f\n"
                   "2:\t" LOAD_USER_SHARED_DATA("%r11")
                   "cmpb $0,0x285(%r11)\n\t"        /* user_shared_data->ProcessorFeatures[PF_XSAVE_ENABLED] */
                   "jz 3f\n\t"
                   "movq %rax,%r11\n\t"
                   LOAD_USER_SHARED_DATA("%rdx")
                   "movl 0x3d8(%rdx),%eax\n\t"      /* user_shared_data->XState.EnabledFeatures */
                   "movl 0x3dc(%rdx),%edx\n\t"
                   "xrstor64 0xc0(%rcx)\n\t"
                   "movq %r11,%rax\n\t"
