...
static BOOL iridium_shared_data_configured;
static void *iridium_teb_block_next;
static BOOL iridium_high_arena_reported;

static BOOL init_iridium_user_shared_data_address(void)
{
    const char *value = getenv( "IRIDIUM_WINE_USER_SHARED_DATA_ADDRESS" );
    char *end;
    unsigned long long address;

    if (!value || !*value) return FALSE;
    address = strtoull( value, &end, 0 );
    if (end == value || *end || (address & host_page_mask)) return FALSE;
    user_shared_data = (void *)(UINT_PTR)address;
    iridium_shared_data_configured = TRUE;
    return TRUE;
}
                          ...
                          ULONG_PTR limit_low, ULONG_PTR limit_high, size_t align_mask )
{
    ...
    else
    {
        ...
        /* iOS occupies most of the low 64-bit address space with native app,
         * dyld, and FEX mappings. The Iridium host retains a guest-owned
         * placeholder above the relocated KUSER_SHARED_DATA/TEB range. Prefer
         * that specific image arena whenever the caller's Windows allocation
         * limits include it, before Wine's generic bottom-up search walks into
         * iOS-owned low ranges. Explicit fixed-address requests still follow
         * the normal path above and fail safely when their preferred address
         * conflicts with the host. */
        if (iridium_shared_data_configured)
        {
            void *iridium_start = (void *)IRIDIUM_GUEST_IMAGE_ARENA_ADDRESS;
            void *iridium_end = (char *)iridium_start + IRIDIUM_GUEST_IMAGE_ARENA_SIZE;
            void *arena_start = max( start, iridium_start );
            void *arena_end = min( end, iridium_end );

            if (arena_start < arena_end &&
                (ptr = map_reserved_area( arena_start, arena_end, host_size,
                                          top_down, unix_prot, align_mask )))
            {
                if (!iridium_high_arena_reported)
                {
                    fprintf( stderr, "iridium-wine: allocating guest memory from high arena %p-%p\n",
                             iridium_start, iridium_end );
                    iridium_high_arena_reported = TRUE;
                }
                TRACE( "got mem in Iridium high arena %p-%p\n", ptr, (char *)ptr + size );
                goto done;
            }
        }
                                     ...
                                     int shared_fd, BOOL removable )
{
    ...
    if (iridium_shared_data_configured)
        fprintf( stderr, "iridium-wine: image view=%p base=%p size=%#lx preferred=%#llx mapped=%#llx name=%s\n",
                 view, ptr, total_size, (unsigned long long)image_info->base,
                 (unsigned long long)image_info->map_addr, debugstr_us(nt_name) );
    ...
    for (i = pos = 0; i < nt->FileHeader.NumberOfSections; i++)
    {
        ...
        if (iridium_shared_data_configured)
            fprintf( stderr, "iridium-wine: section %.8s view=%p base=%p original=%p rva=%#x size=%#lx offset=%#lx\n",
                     sec[i].Name, view, view->base, ptr, sec[i].VirtualAddress, file_size, file_start );
...
static BOOL is_iridium_amd64_dynamic_dll( const struct pe_image_info *image_info )
{
    return iridium_shared_data_configured && image_info->machine == IMAGE_FILE_MACHINE_AMD64 &&
                                   ...
                                   UNICODE_STRING *nt_name, BOOL is_builtin, off_t offset)
{
    ...
    if (is_iridium_amd64_dynamic_dll( image_info ) && image_info->map_addr &&
        !iridium_guest_image_range_is_valid( image_info->map_addr, size ))
    {
        fprintf( stderr,
                 "iridium-wine: discarding PE address %#llx outside validated arena %#llx-%#llx\n",
                 (unsigned long long)image_info->map_addr,
                 (unsigned long long)IRIDIUM_GUEST_IMAGE_ARENA_ADDRESS,
                 (unsigned long long)IRIDIUM_GUEST_IMAGE_ARENA_END );
        image_info->map_addr = 0;
    }
    ...
    if (is_iridium_amd64_dynamic_dll( image_info ))
    {
        if (image_info->map_addr &&
            !iridium_guest_image_range_is_valid( image_info->map_addr, size ))
        {
            fprintf( stderr,
                     "iridium-wine: rejecting server PE address %#llx outside validated arena\n",
                     (unsigned long long)image_info->map_addr );
            image_info->map_addr = 0;
        }

        /* A fixed server address may fail when a previous embedded session
         * left stale per-file placement state. Every fallback must remain in
         * the host-owned placeholder; never let Wine's top-down search escape
         * to Darwin's non-canonical 49-bit allocation range. limit_high is an
         * inclusive API boundary, hence the - 1. */
        limit_low = max( limit_low, (ULONG_PTR)IRIDIUM_GUEST_IMAGE_ARENA_ADDRESS );
        if (!limit_high || limit_high >= IRIDIUM_GUEST_IMAGE_ARENA_END)
            limit_high = IRIDIUM_GUEST_IMAGE_ARENA_END - 1;
        fprintf( stderr, "iridium-wine: constraining AMD64 PE image to %#lx-%#lx\n",
    ...
    /* The normal Unix preloader makes a Wine-server-selected DLL base
     * available, so upstream can treat image_info->map_addr as authoritative.
     * An in-process iOS host cannot promise arbitrary low fixed mappings. If a
     * relocatable image had to fall back into Iridium's validated high arena,
     * relocate its contents to the address that was actually mapped. Without
     * this correction the PE header and relocation delta still point at the
     * failed low address, producing invalid pointers before user32 starts. */
    if (iridium_shared_data_configured &&
        (image_info->image_flags & IMAGE_FLAGS_ImageDynamicallyRelocated) &&
        image_info->map_addr != wine_server_client_ptr( view->base ))
    {
        fprintf( stderr, "iridium-wine: correcting PE relocation base %#llx -> %p\n",
                 (unsigned long long)image_info->map_addr, view->base );
        image_info->map_addr = wine_server_client_ptr( view->base );
    }
...
void virtual_init(void)
{
...
#ifdef _WIN64
    init_iridium_user_shared_data_address();
    host_addr_space_limit = get_host_addr_space_limit();
    if (iridium_shared_data_configured)
    {
        void *iridium_limit = (char *)user_shared_data + IRIDIUM_GUEST_HIGH_ARENA_RESERVE_SIZE;

        /* FEX's in-process Darwin address space is fragmented by the app and
         * dyld mappings. Wine's power-of-two mmap probe can therefore report a
         * limit below a specific high range which the Iridium host has already
         * proved mappable. Extend only far enough to track the validated guest
         * high arena instead of claiming the full Darwin address space. */
        if (host_addr_space_limit < iridium_limit) host_addr_space_limit = iridium_limit;
    }
    ...
    /* The in-process iOS host owns this reservation contract even when an
     * unusual guest preloader path does not publish its preload table. Make
     * the high arena visible to Wine's reserved-area allocator explicitly;
     * mmap_add_reserved_area() merges the normal duplicate declaration. */
    if (iridium_shared_data_configured)
        mmap_add_reserved_area( user_shared_data, IRIDIUM_GUEST_HIGH_ARENA_RESERVE_SIZE );
 ...
 *           alloc_teb_block
 *
 * Allocate a complete TEB block from the host-validated Iridium arena. The
 * normal top-down allocator targets the highest address below the relocated
 * shared-data page, but that range is unavailable in an iOS process. Keeping
 * TEB blocks above the shared page also prevents unrelated allocations from
 * consuming the small amount of high address space Wine needs to start.
 */
static NTSTATUS alloc_teb_block( void **block, SIZE_T *size, ULONG_PTR fallback_limit, ULONG fallback_type )
{
    if (iridium_shared_data_configured)
    {
        void *arena_start = (char *)user_shared_data + IRIDIUM_USER_SHARED_DATA_PREFIX_SIZE;
        void *arena_end = (char *)arena_start + IRIDIUM_TEB_ARENA_SIZE;
        void *base = iridium_teb_block_next ? iridium_teb_block_next : arena_start;
        NTSTATUS status;

        if ((UINT_PTR)base <= (UINT_PTR)arena_end && *size <= (SIZE_T)((char *)arena_end - (char *)base))
        {
            *block = base;
            status = NtAllocateVirtualMemory( NtCurrentProcess(), block, 0, size,
                                              MEM_RESERVE, PAGE_READWRITE );
            if (!status) iridium_teb_block_next = (char *)*block + *size;
            return status;
        }
        return STATUS_NO_MEMORY;
    }

    return NtAllocateVirtualMemory( NtCurrentProcess(), block, fallback_limit, size,
                                    MEM_RESERVE | fallback_type, PAGE_READWRITE );
}
...
TEB *virtual_alloc_first_teb(void)
{
    ...
    if (status)
    {
        ERR( "wine: failed to map the shared user data at %p (size %#lx, host limit %p): %08x\n",
             user_shared_data, data_size, host_addr_space_limit, status );
        exit(1);
    }

    teb_limit = is_win64 && (UINT_PTR)user_shared_data < limit_2g ? limit_2g - 1 : 0;
    status = alloc_teb_block( &teb_block, &total, teb_limit, MEM_TOP_DOWN );
    if (status)
    {
        ERR( "wine: failed to reserve the first TEB block (size %#lx): %08x\n", total, status );
        exit(1);
    }
    teb_block_pos = 30;
    ...
    if (status)
    {
        ERR( "wine: failed to commit the first TEB and PEB (size %#lx): %08x\n", data_size, status );
        exit(1);
    }
...
NTSTATUS virtual_alloc_teb( TEB **ret_teb )
{
    ...
    else
    {
        if (!teb_block_pos)
        {
            ...
            if ((status = alloc_teb_block( &ptr, &total, user_space_wow_limit, 0 )))
            {
                server_leave_uninterrupted_section( &virtual_mutex, &sigset );
                return status;
            }
