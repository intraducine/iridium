...
NTSTATUS get_full_path( char *name, const WCHAR *curdir, UNICODE_STRING *nt_name )
{
    ...
    if (name[0] == '/')
    {
        if (!find_drive_nt_root( name, strlen(name), &ret, FILE_OPEN ) && ret) goto done;

        len = ARRAY_SIZE(unix_prefixW) + strlen(name) + 1;
        if (!(ret = malloc( len * sizeof(WCHAR) ))) return STATUS_NO_MEMORY;
        memcpy( ret, unix_prefixW, sizeof(unix_prefixW) );
        ntdll_umbstowcs( name, strlen(name) + 1, ret + ARRAY_SIZE(unix_prefixW),
                         len - ARRAY_SIZE(unix_prefixW) );
        collapse_path( ret );
        goto done;
    }
