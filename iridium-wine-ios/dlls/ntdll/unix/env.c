...
static const char iridium_skip_wineboot_if_seededA[] = "IRIDIUM_SKIP_WINEBOOT_IF_SEEDED";
static const char wineprefixA[] = "WINEPREFIX";
...
static BOOL iridium_flag_enabled( const char *name )
{
...
static BOOL iridium_seeded_prefix_ready(void)
{
    const char *prefix = getenv( wineprefixA );
    char path[PATH_MAX];

    if (!prefix || !*prefix) return FALSE;

    snprintf( path, sizeof(path), "%s/system.reg", prefix );
    if (access( path, F_OK )) return FALSE;
    snprintf( path, sizeof(path), "%s/user.reg", prefix );
    if (access( path, F_OK )) return FALSE;
    snprintf( path, sizeof(path), "%s/userdef.reg", prefix );
    if (access( path, F_OK )) return FALSE;
    snprintf( path, sizeof(path), "%s/dosdevices/c:", prefix );
    if (access( path, F_OK )) return FALSE;
    snprintf( path, sizeof(path), "%s/dosdevices/z:", prefix );
    if (access( path, F_OK )) return FALSE;
    return TRUE;
}

static BOOL should_skip_iridium_wineboot(void)
{
    if (!iridium_flag_enabled( iridium_skip_wineboot_if_seededA )) return FALSE;
    if (!iridium_seeded_prefix_ready()) return FALSE;
    TRACE( "skipping wineboot for seeded Iridium prefix\n" );
    return TRUE;
}
...
static RTL_USER_PROCESS_PARAMETERS *build_initial_params( void **module )
{
    ...
    if (!should_skip_iridium_wineboot())
    {
    ...
    else is_prefix_bootstrap = !!bootstrap;
