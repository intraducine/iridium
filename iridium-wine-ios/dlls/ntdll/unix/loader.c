...
#define IRIDIUM_SPAWN_HOST_WINESERVER_SYSCALL 0x69726401UL
#define IRIDIUM_WINDOWS_PROCESS_STARTED_SYSCALL 0x69726402UL
...
static int iridium_start_host_wineserver( pid_t *pid, BOOL *embedded, BOOL debug )
{
    const char *path = getenv( "IRIDIUM_WINE_HOST_WINESERVER" );
    long ret;

    *embedded = FALSE;
    /* This private syscall is implemented only by Iridium's in-process FEX
     * handler. Native amd64 Wine builds must keep the standard wineserver
     * execution path instead of issuing an unknown syscall that Linux may
     * reject with EPERM rather than ENOSYS. */
    if (!getenv( "IRIDIUM_FEX_TRANSLATOR_PATH" ))
    {
        errno = ENOSYS;
        return -1;
    }
    ret = syscall( IRIDIUM_SPAWN_HOST_WINESERVER_SYSCALL, path ? path : "", debug ? 1 : 0 );
    if (ret < 0)
    {
        errno = -ret;
        return -1;
...
void start_server( BOOL debug )
{
    ...
    if (!started)
    {
        ...
        argv[2] = NULL;
        if (iridium_start_host_wineserver( &pid, &embedded, debug ))
        {
            int iridium_error = errno;

            if (iridium_error != ENOSYS && iridium_error != ENOENT)
                fatal_error( "embedded wineserver startup failed (errno=%d: %s)\n",
                             iridium_error, strerror( iridium_error ) );
            if (exec_wineserver( &pid, argv ))
            {
                const char *host_server = getenv( "IRIDIUM_WINE_HOST_WINESERVER" );
                const char *guest_server = getenv( "WINESERVER" );
                int error = errno;

                fatal_error(
                    "could not exec wineserver (host=%s guest=%s errno=%d: %s)\n",
                    host_server && host_server[0] ? host_server : "unset",
                    guest_server && guest_server[0] ? guest_server : "unset",
                    error,
                    strerror( error ) );
            }
        }
...
static void start_main_thread(void)
{
    ...
    *(ULONG_PTR *)&peb->CloudFileFlags = get_image_address();
    ...
    syscall( IRIDIUM_WINDOWS_PROCESS_STARTED_SYSCALL );
}
