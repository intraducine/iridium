...
/* Drop every cached session view so subsequent lookups remap current file
 * contents. A cached view can predate the server writes it is asked about
 * (mapped before the server allocated the object or grew the session file),
 * in which case every id comparison against it fails permanently. The
 * initial shared_session view is re-resolved by shared_session_init(). */
static void drop_shared_session_blocks(void)
{
    struct session_block *block, *next;

    pthread_mutex_lock( &session_lock );
    LIST_FOR_EACH_ENTRY_SAFE( block, next, &session_blocks, struct session_block, entry )
    {
        list_remove( &block->entry );
        NtUnmapViewOfSection( GetCurrentProcess(), (void *)block->data );
        free( block );
    }
    shared_session = NULL;
    pthread_mutex_unlock( &session_lock );
...
const shared_object_t *find_shared_session_object( object_id_t id, mem_size_t offset )
{
    ...
    for (;;)
    {
        if ((status = find_shared_session_block( offset, sizeof(*object), &block )))
            return NULL;
        object = (const shared_object_t *)(block->data + offset - block->offset);
        if (id == shared_object_get_id( object )) return object;
        /* A mismatch means the cached view does not reflect the server
         * write this lookup was issued for. Remap once and retry against
         * current contents; a second mismatch is genuine corruption. */
        if (remapped++) break;
        WARN( "Session object id doesn't match expected id %s (observed %s at offset %s), remapping session view\n",
              wine_dbgstr_longlong(id), wine_dbgstr_longlong(shared_object_get_id( object )),
              wine_dbgstr_longlong(offset) );
        drop_shared_session_blocks();
        shared_session_init();
        block = NULL;
    }
