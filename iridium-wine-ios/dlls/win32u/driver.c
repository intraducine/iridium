...
static void iridium_driver_trace( const char *format, ... )
{
    const char *path = getenv( "IRIDIUM_WINE_IOS_TRACE_PATH" );
    FILE *file;
    va_list args;
...
static void iridium_driver_trace_wstr( const char *prefix, const WCHAR *value, ULONG byte_len )
{
    char valueA[MAX_PATH];
    ULONG i, char_len = min( byte_len / sizeof(WCHAR), ARRAY_SIZE(valueA) - 1 );

    if (!prefix || !value) return;
    for (i = 0; i < char_len && value[i]; ++i)
        valueA[i] = value[i] < 0x80 ? (char)value[i] : '?';
    valueA[i] = 0;
    iridium_driver_trace( "%s%s", prefix, valueA );
}

static BOOL load_iridium_environment_driver(void)
{
    const char *driver = getenv( "IRIDIUM_WINE_IOS_GRAPHICS_DRIVER" );
    ...
    if (!driver || !driver[0])
    {
        iridium_driver_trace( "win32u Iridium environment driver missing" );
        return FALSE;
    }
    if (strlen( driver ) >= ARRAY_SIZE(driverW))
    {
        iridium_driver_trace( "win32u Iridium environment driver name too long" );
        return FALSE;
    }

    asciiz_to_unicode( driverW, driver );
    TRACE( "trying Iridium environment driver %s\n", debugstr_w(driverW) );
    iridium_driver_trace( "win32u trying Iridium environment driver %s", driver );
    loaded = !KeUserModeCallback( NtUserLoadDriver, driverW, (strlen( driver ) + 1) * sizeof(WCHAR),
                                  &ret_ptr, &ret_len );
    if (loaded)
    {
        iridium_driver_trace( "win32u Iridium environment driver loaded %s", driver );
        return TRUE;
    }

    asciiz_to_unicode( driver_load_error, "The Iridium graphics driver failed to load." );
    iridium_driver_trace( "win32u Iridium environment driver failed %s", driver );
    return FALSE;
}
...
static BOOL load_desktop_driver( HWND hwnd )
{
    ...
    const char *iridium_driver = getenv( "IRIDIUM_WINE_IOS_GRAPHICS_DRIVER" );
    char buf[4096];
    ...
    if (iridium_driver && iridium_driver[0])
    {
        iridium_driver_trace( "win32u preferring Iridium environment driver over prefix registry" );
        if (load_iridium_environment_driver()) return TRUE;
        iridium_driver_trace( "win32u Iridium environment driver failed; falling back to prefix registry" );
    }
    ...
    {
        iridium_driver_trace( "win32u display driver registry key missing" );
        return load_iridium_environment_driver();
    }
    ...
    if ((size = query_reg_ascii_value( hkey, "GraphicsDriver", info, sizeof(buf) )))
    {
        ...
        iridium_driver_trace_wstr( "win32u registry GraphicsDriver=", (const WCHAR *)info->Data,
                                   info->DataLength );
        if (info->DataLength != sizeof(nullW) || memcmp( info->Data, nullW, sizeof(nullW) ))
        {
            ...
            iridium_driver_trace( "win32u registry GraphicsDriver load %s", ret ? "succeeded" : "failed" );
        }
        else
        {
            __wine_set_user_driver( &null_user_driver, WINE_GDI_DRIVER_VERSION );
            ret = TRUE;
            iridium_driver_trace( "win32u registry selected null display driver" );
        }
    }
    else if ((size = query_reg_ascii_value( hkey, "DriverError", info, sizeof(buf) )))
    {
        ...
        iridium_driver_trace( "win32u registry DriverError present" );
    }
    else
    {
        iridium_driver_trace( "win32u registry GraphicsDriver missing" );
        ret = load_iridium_environment_driver();
    }
...
static void load_display_driver(void)
{
    ...
    iridium_driver_trace( "win32u load_display_driver service=%u lazy=%u",
                          is_service_process(), user_driver == &lazy_load_driver );
    if (is_service_process() || !load_desktop_driver( get_desktop_window() ) || user_driver == &lazy_load_driver)
    {
        iridium_driver_trace( "win32u falling back to null display driver" );
    ...
    else iridium_driver_trace( "win32u display driver active" );
}
