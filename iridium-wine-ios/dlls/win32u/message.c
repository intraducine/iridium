...
/* Iridium's in-process Darwin host reserves the low 4 GiB and maps native
 * images immediately above it. Keep this alias synchronized with the address
 * selected by iridium-fex-ios, in a 16 KiB-aligned upper user-space range. */
static const struct _KUSER_SHARED_DATA *user_shared_data =
    (struct _KUSER_SHARED_DATA *)IRIDIUM_USER_SHARED_DATA_ADDRESS;
