...
DECL_HANDLER(create_class)
{
    ...
    fprintf( stderr, "iridium-wineserver: class atom=%u id=%llu offset=%llu reply_size=%zu\n",
             reply->atom, (unsigned long long)reply->locator.id,
             (unsigned long long)reply->locator.offset, sizeof(*reply) );
}
