...
/* Overridden by the iOS embedded-server archive. */
__attribute__((weak)) void iridium_wineserver_embedded_did_start(void) {}
...
{
    server_exit(1);  /* make sure atexit functions get called */
}
...
int wine_server_main( int argc, char *argv[] )
{
    ...
    /* An embedded server must not replace signal handlers owned by the app. */
