...
{
    if (wine_server_embedded) pthread_exit( (void *)(intptr_t)status );
    exit( status );
}
...
static char *create_server_dir( int force )
{
    ...
    const char *iridium_root = getenv( "IRIDIUM_WINE_SERVER_ROOT" );
    ...
    if (iridium_root && iridium_root[0] == '/')
    {
        if (!(base_dir = strdup( iridium_root ))) fatal_error( "out of memory\n" );
    }
    else
    {
...
static void acquire_lock(void)
{
    ...
    else
    {
        ...
        case EACCES:
            ...
            /* fall through */
        case EAGAIN:
            server_exit(2); /* we didn't get the lock, exit with special status */
        default:
