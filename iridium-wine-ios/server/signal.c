...
void init_signals(void)
{
    ...
    /* The containing iOS app owns process-wide signal disposition. Embedded
     * clients do not create host child processes, so the server pipe objects
     * can remain registered without installing global handlers. */
    if (wine_server_embedded) return;
