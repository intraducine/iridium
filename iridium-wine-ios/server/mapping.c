...
void init_memory(void)
{
    ...
    {
        /* The normal Wine server assigns dynamically relocated PE images from
         * low ranges reserved by the Unix preloader. Those ranges belong to
         * the iOS app and dyld when Wine runs inside Iridium, so every address
         * selected there is guaranteed to fail in the guest. Keep the server's
         * allocation bookkeeping inside the exact placeholder that the host
         * reserved before FEX started. This also makes the server-selected
         * relocation base agree with the address that ntdll will actually map. */
        free_map_addr( IRIDIUM_GUEST_IMAGE_ARENA_ADDRESS, IRIDIUM_GUEST_IMAGE_ARENA_SIZE );
        fprintf( stderr, "iridium-wineserver: PE image arena %#llx-%#llx\n",
                 (unsigned long long)IRIDIUM_GUEST_IMAGE_ARENA_ADDRESS,
                 (unsigned long long)(IRIDIUM_GUEST_IMAGE_ARENA_ADDRESS +
                                      IRIDIUM_GUEST_IMAGE_ARENA_SIZE) );
        return;
    }
#endif
...
#ifdef __APPLE__
    if (wine_server_embedded && mapping->image.machine == IMAGE_FILE_MACHINE_AMD64 &&
        (mapping->image.image_charact & IMAGE_FILE_DLL) && mapping->image.map_addr &&
        !iridium_guest_image_range_is_valid( mapping->image.map_addr,
                                             mapping->image.map_size ))
    {
        fprintf( stderr,
                 "iridium-wineserver: ignoring cached PE address %#llx outside arena %#llx-%#llx\n",
                 (unsigned long long)mapping->image.map_addr,
                 (unsigned long long)IRIDIUM_GUEST_IMAGE_ARENA_ADDRESS,
...
#ifdef __APPLE__
    /* PE32+ Wine DLLs such as opengl32.dll intentionally retain historical
     * preferred bases below 4 GiB. They are still fully relocatable 64-bit
     * images and must use Iridium's 64-bit guest arena instead of ranges32. */
    if (wine_server_embedded && mapping->image.machine == IMAGE_FILE_MACHINE_AMD64)
        range = &ranges64;
    else
#endif
        range = (mapping->image.base >> 32) ? &ranges64 : &ranges32;

    if ((ret = get_fd_map_address( mapping->fd )))
    {
#ifdef __APPLE__
        if (wine_server_embedded && mapping->image.machine == IMAGE_FILE_MACHINE_AMD64 &&
            !iridium_guest_image_range_is_valid( ret, size ))
        {
            fprintf( stderr, "iridium-wineserver: rejecting stale map address %#llx size %#llx\n",
