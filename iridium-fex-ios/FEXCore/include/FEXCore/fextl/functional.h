...
public:
  ...
  move_only_function(F&& f) noexcept(std::is_nothrow_move_constructible_v<F>) {
    if constexpr (std::is_convertible_v<F, R (*)(Args...)>) {
      ...
      // libc++ on Apple toolchains does not provide the same noexcept guarantees here,
      // even for the single-pointer wrapper lambda we use for the heap-backed fallback.
      // Accept the implementation-defined behavior so host-side Darwin builds can
      // still materialize move_only_function callbacks.
