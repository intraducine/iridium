...
template<typename T>
  requires(std::is_integral_v<T> && !std::is_same_v<T, uint8_t> && !std::is_same_v<T, uint16_t> && !std::is_same_v<T, uint32_t> &&
           !std::is_same_v<T, uint64_t>)
static inline T LoadExclusive(T* Futex) {
  if constexpr (sizeof(T) == sizeof(uint8_t)) {
    return static_cast<T>(LoadExclusive(reinterpret_cast<uint8_t*>(Futex)));
  } else if constexpr (sizeof(T) == sizeof(uint16_t)) {
    return static_cast<T>(LoadExclusive(reinterpret_cast<uint16_t*>(Futex)));
  } else if constexpr (sizeof(T) == sizeof(uint32_t)) {
    return static_cast<T>(LoadExclusive(reinterpret_cast<uint32_t*>(Futex)));
  } else {
    static_assert(sizeof(T) == sizeof(uint64_t));
    return static_cast<T>(LoadExclusive(reinterpret_cast<uint64_t*>(Futex)));
  }
}
...
template<typename T>
  requires(std::is_integral_v<T> && !std::is_same_v<T, uint8_t> && !std::is_same_v<T, uint16_t> && !std::is_same_v<T, uint32_t> &&
           !std::is_same_v<T, uint64_t>)
static inline T WFELoadAtomic(T* Futex) {
  if constexpr (sizeof(T) == sizeof(uint8_t)) {
    return static_cast<T>(WFELoadAtomic(reinterpret_cast<uint8_t*>(Futex)));
  } else if constexpr (sizeof(T) == sizeof(uint16_t)) {
    return static_cast<T>(WFELoadAtomic(reinterpret_cast<uint16_t*>(Futex)));
  } else if constexpr (sizeof(T) == sizeof(uint32_t)) {
    return static_cast<T>(WFELoadAtomic(reinterpret_cast<uint32_t*>(Futex)));
  } else {
    static_assert(sizeof(T) == sizeof(uint64_t));
    return static_cast<T>(WFELoadAtomic(reinterpret_cast<uint64_t*>(Futex)));
  }
}
