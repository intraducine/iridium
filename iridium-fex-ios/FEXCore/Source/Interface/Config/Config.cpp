...
static MetaLayer* EnsureMetaLayer() {
  if (Meta != nullptr) {
    return Meta;
  }

  auto Existing = ConfigLayers.find(FEXCore::Config::LayerType::LAYER_TOP);
  if (Existing == ConfigLayers.end()) {
    ConfigLayers.emplace(
      FEXCore::Config::LayerType::LAYER_TOP,
      fextl::make_unique<MetaLayer>(FEXCore::Config::LayerType::LAYER_TOP)
    );
    Existing = ConfigLayers.find(FEXCore::Config::LayerType::LAYER_TOP);
    fprintf(stderr, "[IridiumFEX] Lazily initialized missing FEXCore config MetaLayer\n");
  } else {
    fprintf(stderr, "[IridiumFEX] Recovered null FEXCore config Meta pointer from existing layer map\n");
  }

  Meta = dynamic_cast<MetaLayer*>(Existing->second.get());
  return Meta;
}
...
void ReloadMetaLayer() {
  auto* LocalMeta = EnsureMetaLayer();
  ...
  if (FEXCore::Config::Exists(FEXCore::Config::CONFIG_SINGLESTEP) && LocalMeta->GetConv<bool>(FEXCore::Config::CONFIG_SINGLESTEP).value_or(false)) {
...
bool Exists(ConfigOption Option) {
  auto* LocalMeta = EnsureMetaLayer();
  return LocalMeta != nullptr && LocalMeta->OptionExists(Option);
}
...
std::optional<T> GetConv(ConfigOption Option) {
  ...
  return LocalMeta->GetConv<T>(Option);
}
