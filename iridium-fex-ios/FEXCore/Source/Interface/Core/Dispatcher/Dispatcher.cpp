...
namespace {
std::pair<void*, void*> AllocateDispatcherCodeBuffer() {
  void* buffer = FEXCore::Allocator::VirtualAlloc(MAX_DISPATCHER_CODE_SIZE, true);
  LOGMAN_THROW_A_FMT(buffer != nullptr, "Couldn't allocate dispatcher code buffer");
  return {buffer, FEXCore::Allocator::GetWritableAlias(buffer)};
}
} // namespace

Dispatcher::Dispatcher(FEXCore::Context::ContextImpl* ctx)
  : Dispatcher(ctx, AllocateDispatcherCodeBuffer()) {}

Dispatcher::Dispatcher(FEXCore::Context::ContextImpl* ctx, std::pair<void*, void*> buffers)
  : Arm64Emitter(ctx, buffers.first, MAX_DISPATCHER_CODE_SIZE, buffers.second)
  , CTX {ctx} {
  LOGMAN_THROW_A_FMT(FEXCore::Allocator::ExecuteJITWriteCallbackForCodeMemory(GetBufferBase(), EmitDispatcherCallback, this) == 0,
                     "Failed to populate dispatcher code buffer");

  FEXCore::Allocator::VirtualName("FEXMem_Misc", reinterpret_cast<void*>(GetBufferBase()), MAX_DISPATCHER_CODE_SIZE);
}

int Dispatcher::EmitDispatcherCallback(void* Context) {
  auto* Dispatcher = static_cast<FEXCore::CPU::Dispatcher*>(Context);
  Dispatcher->EmitDispatcher();
  return 0;
}
