...
namespace CPU {
  ...
  CodeBuffer::CodeBuffer(size_t Size)
    ...
    LOGMAN_THROW_A_FMT(Ptr != nullptr, "Couldn't allocate code buffer");
    WritablePtr = static_cast<uint8_t*>(FEXCore::Allocator::GetWritableAlias(Ptr));
    LOGMAN_THROW_A_FMT(WritablePtr != nullptr, "Couldn't resolve writable code buffer view");
