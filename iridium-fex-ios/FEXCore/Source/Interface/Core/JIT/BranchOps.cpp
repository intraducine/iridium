...
DEF_OP(Syscall) {
  ...
  // The syscall handler result is returned in x0, but filling static state below
  // uses x0 internally for metadata updates. Preserve the result in a non-SRA
  // scratch register until the guest destination can be updated.
  mov(ARMEmitter::Size::i64Bit, TMP4, ARMEmitter::Reg::r0);
  ...
  if (OSABI != FEXCore::HLE::SyscallOSABI::OS_GENERIC) {
    ...
    mov(ARMEmitter::Size::i64Bit, GetReg(Node), TMP4);
  }
}
