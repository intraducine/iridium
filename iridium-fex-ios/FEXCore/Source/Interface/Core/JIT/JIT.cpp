...
static void DirectBlockDelinker(FEXCore::Context::ExitFunctionLinkData* Record, bool Call) {
  ...
  FEXCore::Allocator::StoreToCodeMemory(reinterpret_cast<uint32_t*>(CallerAddress), BranchInst);
...
static void IndirectBlockDelinker(FEXCore::Context::ExitFunctionLinkData* Record) {
  ...
  FEXCore::Allocator::StoreToCodeMemory(reinterpret_cast<uint32_t*>(JumpThunkStartAddress), BranchInst);
...
uint64_t Arm64JITCore::ExitFunctionLink(FEXCore::Core::CpuStateFrame* Frame, FEXCore::Context::ExitFunctionLinkData* Record) {
  ...
  if (ARMEmitter::Emitter::IsInt26(BranchOffset)) {
    ...
    FEXCore::Allocator::StoreToCodeMemory(Record->HostCode, HostCode);
    ...
    FEXCore::Allocator::StoreToCodeMemory(reinterpret_cast<uint32_t*>(JumpThunkStartAddress), LdrInst);
    ARMEmitter::Emitter::ClearICache(reinterpret_cast<void*>(JumpThunkStartAddress), 4);
...
void Arm64JITCore::ClearCache() {
  ...
  SetBuffer(CodeBuffer->Ptr, CodeBuffer->WritablePtr, CodeBuffer->AllocatedSize);
  const auto EmitDetectionStringCallback = [](void* context) -> int {
    static_cast<Arm64JITCore*>(context)->EmitDetectionString();
    return 0;
  };
  LOGMAN_THROW_A_FMT(FEXCore::Allocator::ExecuteJITWriteCallbackForCodeMemory(CodeBuffer->Ptr, EmitDetectionStringCallback, this) == 0,
                     "Failed to emit the JIT detection string");
...
void Arm64JITCore::EmitSuspendInterruptCheck() {
  if (CTX->Config.NeedsPendingInterruptFaultCheck) {
    ...
    StoreZeroToInterruptFaultPage(TMP2);
  }
...
#undef REGISTER_OP
  ...
  // This can block progress in other compiling threads, so the duration of the lock should be as small as possible.
  {
    ...
    // Bring CodeBuffer up to date
    {
      ...
      SetBuffer(CurrentCodeBuffer->Ptr, CurrentCodeBuffer->WritablePtr, CurrentCodeBuffer->AllocatedSize);
    ...
    FEXCore::Allocator::MemcpyToCodeMemory(GetCursorAddress<uint8_t*>(), TempCodeBuffer, TempSize);
