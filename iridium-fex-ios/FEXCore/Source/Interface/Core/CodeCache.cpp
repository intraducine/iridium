...
namespace {
struct ApplyCodeRelocationsContext {
  FEXCore::Context::CodeCache* Cache;
  uint64_t GuestDelta;
...
int ApplyCodeRelocationsCallback(void* Opaque) {
  auto* Context = static_cast<ApplyCodeRelocationsContext*>(Opaque);
  Context->Result = Context->Cache->ApplyCodeRelocations(Context->GuestDelta, Context->Code, Context->Relocations, Context->ForStorage);
  return 0;
}

bool ApplyCodeRelocationsInWriteScope(
  FEXCore::Context::CodeCache& Cache,
  uint64_t GuestDelta,
  std::span<std::byte> Code,
  std::span<const FEXCore::CPU::Relocation> Relocations,
  ...
  ApplyCodeRelocationsContext Context {
    .Cache = &Cache,
  ...
  FEXCore::Allocator::ExecuteJITWriteCallbackForCodeMemory(Code.data(), ApplyCodeRelocationsCallback, &Context);
...
bool CodeCache::SaveData(Core::InternalThreadState& Thread, int fd, const ExecutableFileSectionInfo& SourceBinary, uint64_t SerializedBaseAddress) {
  ...
  while (Off != AlignUp(Off, Utils::FEX_PAGE_SIZE)) {
    auto BytesToWrite = std::min<uint64_t>(AlignUp(Off, Utils::FEX_PAGE_SIZE) - Off, sizeof(Zero));
  ...
  if (!ApplyCodeRelocationsInWriteScope(*this, SerializedBaseAddress, CodeBufferData, Relocations, true)) {
...
bool CodeCache::LoadData(Core::InternalThreadState* Thread, std::byte* MappedCacheFile, const ExecutableFileSectionInfo& BinarySection) {
  ...
  FEXCore::Allocator::MemcpyToCodeMemory(CodeBufferRange.data(), MappedCacheFile, header.CodeBufferSize);
  MappedCacheFile += header.CodeBufferSize;
  CTX.LatestOffset += header.CodeBufferSize;

  // Apply FEX relocations
  auto Ret = ApplyCodeRelocationsInWriteScope(*this, BinarySection.FileStartVA, CodeBufferRange, Relocations, false);
  LOGMAN_THROW_A_FMT(Ret == true, "Failed to apply code cache relocations");
  ARMEmitter::Emitter::ClearICache(CodeBufferRange.data(), CodeBufferRange.size_bytes());
...
void CodeCache::Validate(const ExecutableFileSectionInfo& Section, fextl::set<uint64_t> GuestBlocks, const fextl::set<uint64_t>& HostBlocks,
  ...
  (void)ApplyCodeRelocationsInWriteScope(*this, Section.FileStartVA, CodeBufferRangeRef, NewRelocations, false);
  ...
  if (Mismatch != CodeBufferRangeRef.end()) {
    ...
    ERROR_AND_DIE_FMT(
      "Cache validation failed at offset {:#x}: {:02x}{:02x}{:02x}{:02x} <-> {:02x}{:02x}{:02x}{:02x} "
      "(at {} <-> {}, guest block {})",
      Idx, std::to_integer<unsigned>(CachedCode[Idx]), std::to_integer<unsigned>(CachedCode[Idx + 1]),
      std::to_integer<unsigned>(CachedCode[Idx + 2]), std::to_integer<unsigned>(CachedCode[Idx + 3]),
      std::to_integer<unsigned>(CodeBufferRangeRef[Idx]), std::to_integer<unsigned>(CodeBufferRangeRef[Idx + 1]),
      std::to_integer<unsigned>(CodeBufferRangeRef[Idx + 2]), std::to_integer<unsigned>(CodeBufferRangeRef[Idx + 3]),
