...
public:
  ...
  Emitter(uint8_t* Base, uint8_t* WritableBase, uint64_t BaseSize)
    : Buffer(Base, WritableBase, BaseSize) {}
  ...
  [[nodiscard]] bool Bind(const ForwardLabel::Reference* Label) {
    ...
    case ForwardLabel::InstType::ADR: {
      const auto* Instruction = reinterpret_cast<uint32_t*>(Label->Location);
      auto* WritableInstruction = reinterpret_cast<uint32_t*>(TranslateWriteAddress(Instruction));
