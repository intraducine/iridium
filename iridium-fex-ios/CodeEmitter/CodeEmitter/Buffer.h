...
public:
  ...
  Buffer(uint8_t* Base, uint8_t* WritableBase, uint64_t BaseSize) {
    SetBuffer(Base, WritableBase, BaseSize);
  }

  void SetBuffer(uint8_t* Base, uint64_t BaseSize) {
    SetBuffer(Base, Base, BaseSize);
  }

  void SetBuffer(uint8_t* Base, uint8_t* WritableBase, uint64_t BaseSize) {
    BufferBase = Base;
    WriteBase = WritableBase != nullptr ? WritableBase : Base;
  ...
  void dcn(const T& Data) {
    std::memcpy(GetWritableCursor(), &Data, sizeof(Data));
  ...
  void EmitString(const char* String) {
    ...
    std::memcpy(GetWritableCursor(), String, StringLength);
  ...
  void Align(size_t Size = 4) {
    ...
    std::memset(GetWritableCursor(), 0, Size - CurrentAlignment);
...
protected:
  uint8_t* TranslateWriteAddress(const void* Address) const {
    const auto* CurrentAddress = reinterpret_cast<const uint8_t*>(Address);
    if (CurrentAddress == nullptr || BufferBase == nullptr || WriteBase == nullptr || BufferBase == WriteBase) {
      return const_cast<uint8_t*>(CurrentAddress);
    }

    return WriteBase + (CurrentAddress - BufferBase);
  }

  uint8_t* GetWritableCursor() const {
    return TranslateWriteAddress(CurrentOffset);
  }
