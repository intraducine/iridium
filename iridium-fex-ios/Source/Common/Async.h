...
public:
  ...
  error run_one(std::optional<std::chrono::nanoseconds> Timeout = std::nullopt) {
    ...
    while (true) {
      int Result = wait_for_events(Timeout);
...
private:
  ...
  int wait_for_events(std::optional<std::chrono::nanoseconds> Timeout) {
#if defined(__linux__)
    timespec ts = to_timespec(Timeout.value_or(std::chrono::nanoseconds {0}));
    return ::ppoll(PollFDs.data(), PollFDs.size(), Timeout ? &ts : nullptr, nullptr);
#else
    int TimeoutMS = -1;
    if (Timeout) {
      auto Milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(*Timeout);
      TimeoutMS = Milliseconds.count() > INT_MAX ? INT_MAX : static_cast<int>(Milliseconds.count());
    }
    return ::poll(PollFDs.data(), PollFDs.size(), TimeoutMS);
#endif
  }
