...
inline bool CopyFile(const fextl::string& From, const fextl::string& To, CopyOptions Options = CopyOptions::NONE) {
  ...
  if (Options == CopyOptions::OVERWRITE_EXISTING && DestExists) {
    ...
    bool Result = false;
#if defined(__linux__)
    Result = sendfile(DestinationFD, SourceFD, nullptr, buf.st_size) == buf.st_size;
#else
    constexpr size_t CopyBufferSize = 1U << 16;
    char CopyBuffer[CopyBufferSize];
    Result = true;

    while (true) {
      ssize_t ReadSize = read(SourceFD, CopyBuffer, sizeof(CopyBuffer));
      if (ReadSize == 0) {
        break;
      }
      if (ReadSize < 0) {
      ...
      while (Written < ReadSize) {
        ssize_t WriteSize = write(DestinationFD, CopyBuffer + Written, ReadSize - Written);
        if (WriteSize <= 0) {
          Result = false;
          break;
        }
