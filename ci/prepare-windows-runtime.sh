...
# Full Wine build supplies both PE architectures, import libraries, fonts/NLS,
# and the host loader used only to initialize a new temporary prefix.
...
PY
    ...
    for module in d3d11/d3d11 dxgi/dxgi winemetal/winemetal d3d10/d3d10core; do
        cp "$build/src/$module.dll" "$APP/$arch-windows/"
    done
done
