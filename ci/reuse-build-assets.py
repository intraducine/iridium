...
def validate(run, jobs, stage, branch):
    ...
    if stage == 'linux-userland':
        linux.validate_run(run, jobs, revision, branch)
          ...
          or not any(j.get('name') == stage and j.get('conclusion') == 'success' for j in jobs)):
        raise ValueError('Media producer must be a successful manual job on main or this branch')
    return revision


def compatible(root, revision, stage):
    subprocess.run(['git', '-C', str(root), 'fetch', '--quiet', '--depth=1', 'origin', revision], check=True)
    paths = MEDIA_INPUTS if stage == 'media' else linux.INPUTS + ('check-public-source.py',)
    for path in paths:
...
def verify_producer(root, run_id, stage, branch):
    ...
    name = 'media-sdk-with-source' if stage == 'media' else 'linux-runtime-with-source'
